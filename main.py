...
def PlayPolsat(stream_url,data):
    ...
    if is_helper.check_inputstream():
        ...
        play_item.setProperty('inputstream.adaptive.stream_headers', 'Referer: https://go.cyfrowypolsat.pl')
...
def checkAccess(id):
    ...
    response = requests.post('https://b2c.redefine.pl/rpc/payments/', headers=headers, json=data,timeout=15).json()
...
def playVOD(id):
    ...
    if dostep:
        ...
        xbmcgui.Dialog().notification('[B]Uwaga[/B]', 'Brak dostępu do materiału',xbmcgui.NOTIFICATION_INFO, 6000)
        return False
def playCPGO(id,cpid=0):
    ...
    data = {"jsonrpc":"2.0","method":"getSession","id":1,"params":{"ua":"cpgo_www/2015","authData":{"sessionToken":authdata}}}
    
    response = requests.post(auth_url, headers=headers, json=data,timeout=15).json()
    ...
    authdata=getHmac(dane)
    data = {"jsonrpc":"2.0","id":1,"method":"prePlayData","params":{"ua":"cpgo_www_html5/2 (Windows 10; widevine=true)","cpid":cpid,"mediaId":id,"authData":{"sessionToken":authdata}}}
    ...
    try:
        ...
        response = requests.post('https://b2c.redefine.pl/rpc/drm/', headers=headers, json=data,timeout=15).json()
        ...
        authdata=getHmac(dane)
        devcid=devid.replace('-','')
        data=quote('{"jsonrpc":"2.0","id":1,"method":"getWidevineLicense","params":{"cpid":%d,"mediaId":"'%cpid+mediaid+'","sourceId":"'+sourceid+'","keyId":"'+keyid+'","object":"b{SSM}","deviceId":{"type":"other","value":"'+devcid+'"},"ua":"cpgo_www_html5/2","authData":{"sessionToken":"'+authdata+'"}}}')
...
def PlayPolsatPseudo(str_url):
    ...
    play_item.setArt({'thumb': dane['img'], 'poster': dane['img'], 'banner': dane['img'], 'fanart': FANART})
...
def home():
    logged=loginCPgo()
    if logged:
        add_item(name='Zalogowany', url='', mode=' ', image='DefaultUser.png', folder=False, isPlayable=False,FANART=FANART)
        add_item(name='Telewizja', url='', mode='tvcpgo', image='', folder=True, isPlayable=False,FANART=FANART)
        add_item(name='VOD', url='', mode='vodmain', image='', folder=True, isPlayable=False,FANART=FANART)
    else:
...
def getEpgs():
    ...
    data={"jsonrpc":"2.0","method":"getChannelsProgram","id":1,"params":{"channelIds":kanaly,"fromDate":aa1,"toDate":aa,"ua":"cpgo_www/2015","authData":{"sessionToken":authdata}}}
    response = requests.post(navigate_url, headers=headers, json=data,timeout=15).json()
...
def vodmain():
    ...
    add_item(name='HBO KIDS BAJKI', url='5013942', mode='vodlist', image='https://r.dcs.redcdn.pl/http/o2/redefine/redb/j9/j9p3zzbbmvdmh9chn5q6getonjap53es.jpg', folder=True, isPlayable=False,FANART=FANART)
    add_item(name='HBO KIDS FILMY', url='5013941', mode='vodlist', image='https://r.dcs.redcdn.pl/http/o2/redefine/redb/dz/dz71b4398yukkaun4mjym6ifdqc9acsv.jpg', folder=True, isPlayable=False,FANART=FANART)
    add_item(name='HBO SERIALE', url='5013940', mode='vodlist', image='https://r.dcs.redcdn.pl/http/o2/redefine/redb/47/47oujwhuddyeu2fzmzai4eaqkad1nexu.jpg', folder=True, isPlayable=False,FANART=FANART)
    add_item(name='HBO FILMY', url='5013939', mode='vodlist', image='https://r.dcs.redcdn.pl/http/o2/redefine/redb/m8/m8scw85v5cu6q2ojo4mv54c9xhy8qmdh.jpg', folder=True, isPlayable=False,FANART=FANART)
    add_item(name='SERIAL', url='5006931', mode='vodlist', image='https://r.dcs.redcdn.pl/http/o2/redefine/redb/64/6443791fd5030a9be3114f72ab366ff5.jpg', folder=True, isPlayable=False,FANART=FANART)
    add_item(name='SPORT', url='5006453', mode='vodlist', image='https://r.dcs.redcdn.pl/http/o2/redefine/redb/d4/d4ce016f9f8c11eb1f6c14da33ceb965.jpg', folder=True, isPlayable=False,FANART=FANART)
    add_item(name='WIEDZA', url='5005992', mode='vodlist', image='https://r.dcs.redcdn.pl/http/o2/redefine/redb/16/161e7fa69332ebfd9d06688cfc415a70.jpg', folder=True, isPlayable=False,FANART=FANART)
    add_item(name='ROZRYWKA', url='5005431', mode='vodlist', image='https://r.dcs.redcdn.pl/http/o2/redefine/redb/3b/3bae15a5a5009189bdd40e54ad99f1e8.jpg', folder=True, isPlayable=False,FANART=FANART)
    add_item(name='NEWS', url='5005294', mode='vodlist', image='https://r.dcs.redcdn.pl/http/o2/redefine/redb/61/61e3597b651a7b223fe2c52a753d33eb.jpg', folder=True, isPlayable=False,FANART=FANART)
    add_item(name='DZIECI', url='5005051', mode='vodlist', image='https://r.dcs.redcdn.pl/http/o2/redefine/redb/8d/8d10fc3ff57b5019a8b546836dfbbd8f.jpg', folder=True, isPlayable=False,FANART=FANART)
    add_item(name='FILM', url='5005049', mode='vodlist', image='https://r.dcs.redcdn.pl/http/o2/redefine/redb/a5/a548757da60de0894d63a6f91becd5a3.jpg', folder=True, isPlayable=False,FANART=FANART)
    xbmcplugin.endOfDirectory(addon_handle)
...
def vodSzukaj(query):
    ...
    data={"jsonrpc":"2.0","method":"searchContentWithTreeNavigation","id":1,"params":{"query":query,"limit":50,"ua":"cpgo_www/2015","authData":{"sessionToken":authdata}}}
    response = requests.post(navigate_url, headers=headers, json=data,timeout=15).json()
    #
    mud='playVOD'
    folder=False
    ...
    for item in items:
        ...
        add_item(name=item.get('title'), url=item.get('id'), mode=mud, image=item.get('img'), folder=folder, isPlayable=isplay, infoLabels=item, itemcount=itemz,FANART=FANART)    
    xbmcplugin.endOfDirectory(addon_handle)
...
def vodList(id):
    ...
    authdata=getHmac(dane)
    data = {"jsonrpc":"2.0","method":"getCategoryContentWithFlatNavigation","id":1,"params":{"catid":int(id),"limit":50,"offset":int(offse),"collection":{"type":"sortedby","name":"A-Z","default":True,"value":"13"},"ua":"cpgo_www/2015","authData":{"sessionToken":authdata}}}
    ...
    if int(offse)+50<otal:
        add_item(name='Następna strona', url=id, mode='vodlist', image=RESOURCES+'nextpage.png', folder=True, isPlayable=False, infoLabels=None, itemcount=itemz,page=int(offse)+50,FANART=FANART)    
